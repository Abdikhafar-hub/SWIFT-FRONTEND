...
export const AdminGovernmentSubmissionModal: React.FC<AdminGovernmentSubmissionModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-4xl overflow-hidden rounded-xl border border-amber-500/20 bg-slate-900 text-slate-100 shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-800 bg-slate-950/80 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold tracking-wide text-white">Register Government Submission</h2>
              <p className="text-xs text-slate-400">Statutory Agency Lodgement & Operations Dossier Initiation</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-3 border-b border-slate-800 bg-slate-950/40 text-xs">
          <div
            className={`flex items-center gap-2 border-r border-slate-800 px-6 py-3 font-semibold ${
              step === 1 ? "bg-amber-500/10 text-amber-400 border-b-2 border-b-amber-500" : "text-slate-400"
            }`}
          >
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-slate-800 text-xs">1</span>
            Application & Readiness Audit
          </div>
          <div
            className={`flex items-center gap-2 border-r border-slate-800 px-6 py-3 font-semibold ${
              step === 2 ? "bg-amber-500/10 text-amber-400 border-b-2 border-b-amber-500" : "text-slate-400"
            }`}
          >
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-slate-800 text-xs">2</span>
            Registry & Statutory Details
          </div>
          <div
            className={`flex items-center gap-2 px-6 py-3 font-semibold ${
              step === 3 ? "bg-amber-500/10 text-amber-400 border-b-2 border-b-amber-500" : "text-slate-400"
            }`}
          >
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-slate-800 text-xs">3</span>
        ...
        <div className="max-h-[70vh] overflow-y-auto p-6 space-y-6">
            ...
            <div className="space-y-6">
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-slate-700 bg-slate-950 px-4 py-3 text-sm text-white focus:border-amber-500 focus:outline-none"
                >
              ...
              {selectedApp && (
                <div className="rounded-xl border border-slate-800 bg-slate-950 p-5 space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                    <div>
                      <span className="text-xs text-amber-400 font-mono font-bold">{selectedApp.applicationNumber}</span>
                      <h3 className="text-base font-bold text-white">{selectedApp.service.name}</h3>
                      <p className="text-xs text-slate-400">Client: {selectedApp.client.fullName} ({selectedApp.client.email})</p>
                    </div>
                    <div className="text-right">
                      ...
                      >
                        ...
                        Readiness Score: {selectedApp.readiness.score}%
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">Prerequisite Checklist Verification</h4>
                    ...
                    {!selectedApp.readiness.ready && (
                      <div className="rounded-lg border border-rose-500/30 bg-rose-500/10 p-3 text-xs text-rose-300 space-y-1">
                        <div className="font-bold flex items-center gap-1.5">
                          <AlertTriangle className="h-4 w-4" /> Blockers Identified:
                        </div>
                  ...
                  {!selectedApp.readiness.ready && (
                    <div className="flex items-center gap-3 pt-2 border-t border-slate-800">
                      ...
                      />
                      <label htmlFor="override" className="text-xs text-amber-400 font-medium cursor-pointer">
          ...
          {step === 2 && (
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Government Platform *</label>
                <select
                  value={platform}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Statutory Agency Name *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Submission Channel</label>
                <select
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">External Reference / Application # *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Tracking / Acknowledgment Number</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Statutory Fee Receipt #</label>
                <input
                  type="text"
                  value={receiptNumber}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Statutory Payment Status</label>
                <select
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Statutory Fee Amount (KES)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Expected Turnaround (Days)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Registry Portal URL</label>
                <input
                  type="url"
                  value={portalUrl}
          ...
          {step === 3 && (
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Operations Team / Unit</label>
                  <input
                    type="text"
                    value={team}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Acknowledgement Receipt Document URL</label>
                  <input
                    type="url"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Internal Operations Notes & Submission Checklist</label>
                <textarea
                  rows={4}
                  value={notes}
        ...
        <div className="flex items-center justify-between border-t border-slate-800 bg-slate-950/80 px-6 py-4">
          {step > 1 ? (
            <button
              ...
              className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-900 px-4 py-2 text-xs font-semibold text-slate-300 hover:bg-slate-800 hover:text-white"
            >
          ...
          {step < 3 ? (
            <button
              ...
              onClick={() => setStep((s) => (s + 1) as any)}
              className="flex items-center gap-2 rounded-lg bg-amber-500 px-5 py-2 text-xs font-bold text-slate-950 hover:bg-amber-400 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-lg bg-emerald-500 px-6 py-2 text-xs font-bold text-slate-950 hover:bg-emerald-400 disabled:opacity-50"
            >

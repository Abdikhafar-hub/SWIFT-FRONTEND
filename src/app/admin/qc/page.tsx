...
import { AdminQcModal, AdminStartQcModal } from "@/components/domain";
...
export default function AdminQcPage() {
  ...
  const { data: metrics } = useQuery({
    queryKey: ["admin-qc-metrics"],
    queryFn: () => adminApi.getQcMetrics(),
  });
  ...
  const {
    ...
    queryKey: ["admin-qc-queue", page, search, statusFilter, priorityFilter],
    queryFn: () =>
      adminApi.getQcQueue({
        page,
        limit: 20,
  ...
  const applications = queueData?.items || [];
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 p-4 sm:p-5 lg:p-6 space-y-4 max-w-[1550px] mx-auto font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-1 border-b border-slate-200/60">
        <div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
            Quality Control Operations Center
          </h1>
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            Rigorous statutory compliance inspection, document legibility audits, deficiency management, and formal sign-offs.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className="px-3 py-2 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs rounded-xl shadow-2xs transition-all flex items-center gap-1.5"
          >
            <UserCheck className="size-4 text-slate-500" />
            <span>Assign QC Work</span>
          </button>

          <button
            onClick={() => setIsStartModalOpen(true)}
            className="px-3.5 py-2 bg-gradient-to-r from-[#C5A059] to-[#D4AF37] hover:from-[#b49049] hover:to-[#c39e26] text-white font-extrabold text-xs rounded-xl shadow-xs transition-all flex items-center gap-1.5"
          >
            <Plus className="size-4 stroke-[3]" />
            <span>Start QC Inspection</span>
          </button>
        </div>
      </div>

      {/* ------------------------------------------------------------------ */}
      {/* 2. DYNAMIC QC METRICS */}
      {/* ------------------------------------------------------------------ */}
      ...
      <div className="bg-white rounded-xl p-3.5 border border-slate-200/80 shadow-[0_2px_8px_rgba(0,0,0,0.02)] flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 w-full md:w-auto">
          <select
            ...
            className="w-full sm:w-48 px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all font-semibold text-slate-700"
          >
            <option value="ALL">All Workstream Statuses</option>
            <option value="PENDING">QC Pending / In Progress</option>
            <option value="PASSED">Certified QC Passed</option>
            <option value="RETURNED">Returned to Client</option>
            <option value="FAILED">Flagged / On Hold</option>
          </select>
          ...
          <select
            ...
            className="w-full sm:w-40 px-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 transition-all font-semibold text-slate-700"
          >
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-[0_2px_8px_rgba(0,0,0,0.02)] overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center space-y-2">
            <h3 className="text-sm font-bold text-slate-800">No matching candidate applications</h3>
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              All candidate dossiers have undergone quality checks or match current filter parameters.
            </p>
          </div>
        ) : (
          <div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 text-xs">
                  {applications.map((app: any) => {
                    const progress = app.progress || { satisfied: 0, totalRequirements: 0, failed: 0 };
                    const progressPercent = progress.totalRequirements > 0
                      ? Math.round((progress.satisfied / progress.totalRequirements) * 100)
                      : 0;
                      ...
                      <tr key={app.id} className="hover:bg-slate-50/80 transition-colors group">
                        <td className="py-3 px-4">
                          ...
                          {app.priority === "URGENT" || app.priority === "HIGH" ? (
                            <span className="ml-1.5 text-[9px] font-extrabold px-1.5 py-0.5 rounded bg-rose-50 text-rose-700 border border-rose-200">
                              {app.priority}
                            </span>
                          ) : null}
                        </td>
                          ...
                          <span
                            className={`text-[10px] font-bold px-2 py-0.5 rounded-md border ${
                              app.status === "QUALITY_CHECK"
                                ...
                                : app.status === "READY_FOR_DELIVERY" || app.status === "READY_FOR_SUBMISSION" || app.status === "DELIVERED"
                        ...
                        <td className="py-3 px-4 text-xs">
                          <div className="w-36 space-y-1">
                            <div className="flex items-center justify-between text-[10px] text-slate-500 font-medium">
                              <span>{progress.satisfied} / {progress.totalRequirements} Satisfied</span>
      ...
      {/* START QC INSPECTION INITIALIZATION MODAL */}
      <AdminStartQcModal
